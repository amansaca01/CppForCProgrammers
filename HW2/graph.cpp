...
graph::graph(const int &size, const float &density, const pairs &range) : size(size), density(density), range(range) {
...
void graph::add_edge(int x, int y, int distance) {
    if (!adjacent(x, y) && x != y && distance <= range.second && distance >= range.first) {
